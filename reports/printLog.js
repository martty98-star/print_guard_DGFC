((global) => {
  ...
  function derivePrintLifecycleStatus(attempts) {
    const results = attempts.map((attempt) =>
      normalizePrintLogResult(attempt.result),
    );
    const doneIndex = results.reduce(
      (acc, result, index) => (result === 'done' ? index : acc),
      -1,
    );
    ...
    const failBeforeDone =
      ...
      results
        .slice(0, doneIndex)
        .some((result) => result === 'deleted' || result === 'abrt');
    ...
    if (
      hasDone &&
      doneCount > 1 &&
      !results.some((result) => result === 'deleted' || result === 'abrt')
    )
    ...
    if (!hasDone && deletedCount === attempts.length && attempts.length)
      return 'deleted_only';
    if (!hasDone && abortedCount === attempts.length && attempts.length)
      return 'aborted_only';
  ...
  function buildPrintLifecycleGroups(rows, options) {
    ...
    const sorted = [...(rows || [])].sort(
      (a, b) => new Date(a.readyAt) - new Date(b.readyAt),
    );
    const groups = [];
    ...
    sorted.forEach((row) => {
      ...
      if (
        ...
        !Number.isFinite(group.lastReadyMs) ||
        readyMs - group.lastReadyMs > gapMs
      ) {
        group = {
    ...
    return groups
      .map((group) => {
        ...
        const successfulAttempts = attempts.filter(
          (attempt) => normalizePrintLogResult(attempt.result) === 'done',
        );
        ...
        return {
          ...
          totalPrintedAreaM2: attempts.reduce(
            (sum, attempt) => sum + (Number(attempt.printedAreaM2) || 0),
            0,
          ),
          mediaLengthM: attempts.reduce(
            (sum, attempt) => sum + (Number(attempt.mediaLengthM) || 0),
            0,
          ),
          totalDurationSec: attempts.reduce(
            (sum, attempt) => sum + (Number(attempt.durationSec) || 0),
            0,
          ),
  ...
  function buildPrintErrorSummary(groups) {
    ...
    const firstPass = rows.filter(
      (group) => group.lifecycleStatus === 'success_first_try',
    );
    const resolvedRetries = rows.filter(
      (group) => group.lifecycleStatus === 'resolved_after_retry',
    );
    const unresolved = rows.filter((group) =>
      ['open_issue', 'deleted_only', 'aborted_only', 'unresolved'].includes(
        group.lifecycleStatus,
      ),
    );
    const avgAttempts = rows.length
      ? rows.reduce((sum, group) => sum + group.attemptCount, 0) / rows.length
      : 0;
    const avgAttemptsSuccess = successful.length
      ? successful.reduce((sum, group) => sum + group.attemptCount, 0) /
        successful.length
      : 0;
  ...
  function buildPrintLogSummary(summary, options) {
    ...
    Object.entries(byPrinter).forEach(([printerName, record]) => {
      ...
      if (!Number.isFinite(area) || area <= 0 || !Number.isFinite(ratio))
        return;
      estimatedInk += area * ratio;
      hasEstimate = true;
    });

    const directInk = summary?.inkDataAvailable
      ? Number(summary?.inkTotalL)
      : null;
    ...
    return {
      ...
      inkL: Number.isFinite(directInk)
        ? directInk
        : hasEstimate
          ? estimatedInk
          : null,
      inkSource: Number.isFinite(directInk)
        ? 'direct'
        : hasEstimate
          ? 'estimated'
          : null,
