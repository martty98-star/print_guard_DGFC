(function attachChecklistDomain(root, factory) {
  ...
  function createChecklistDomain() {
    'use strict';
    ...
    function validateChecklistItemInput(input, now) {
      ...
      const scheduleType =
        String((input && input.scheduleType) || '')
          ...
          ? 'monthly'
          : 'weekly';
      const daysOfWeek = normalizeChecklistDaysOfWeek(
        input && input.daysOfWeek,
      );
      const dayOfMonth = normalizeChecklistDayOfMonth(
        input && input.dayOfMonth,
      );
      ...
      return {
        ...
        createdAt:
          cleanOptionalString(input && input.createdAt) ||
          current.toISOString(),
        updatedAt:
          cleanOptionalString(input && input.updatedAt) ||
          current.toISOString(),
        createdBy: cleanOptionalString(input && input.createdBy),
        updatedBy: cleanOptionalString(input && input.updatedBy),
        scheduleType,
        timeZone:
          cleanOptionalString(input && input.timeZone) || 'Europe/Prague',
      };
    }
    ...
    function evaluateDueChecklistOccurrences(items, options) {
      ...
      for (const rawItem of items || []) {
        ...
        for (const localDate of candidateDates) {
          ...
          if (
            occurrencePseudo > nowPseudo ||
            occurrencePseudo < windowStartPseudo
          )
            continue;
          occurrences.push({
            occurrenceKey: buildChecklistOccurrenceKey(
