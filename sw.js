const CACHE_NAME = 'printguard-v8.1.3';
    ...
    caches
      ...
      .then((keys) =>
        Promise.all(
          keys
            .filter(
              (key) => key !== CACHE_NAME && key.startsWith('printguard-'),
            )
      ...
      try {
        ...
        for (const client of clients) {
          ...
          if (typeof client.focus === 'function') {
            try {
              await client.focus();
              console.log('[SW] notificationclick reused existing client', {
                targetUrl,
              });
              return;
            } catch (error) {
        ...
        try {
          ...
          console.log('[SW] notificationclick opened new window', {
            targetUrl,
          });
        } catch (error) {
