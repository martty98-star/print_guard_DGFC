...
function cleanApiError(error) {
  const message =
    error && error.message ? error.message : 'order-pipeline failed';
...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    const requestUrl =
      event.rawUrl || event.path || '/.netlify/functions/order-pipeline';
    const body = await withClient(
      async (client) => {
        if (
          query.detail === '1' ||
          query.detail === 'true' ||
        ...
        const stats =
          includeStats.global || includeStats.scope
