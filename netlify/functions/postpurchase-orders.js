...
function parseUpdateOptions(event) {
  ...
  if (
    Object.hasOwn(body, 'reprintNeeded') ||
    Object.hasOwn(body, 'reprint_needed')
  ) {
    options.reprintNeeded = body.reprintNeeded ?? body.reprint_needed;
  }
  if (
    Object.hasOwn(body, 'issueReason') ||
    Object.hasOwn(body, 'issue_reason')
  ) {
    options.issueReason = body.issueReason ?? body.issue_reason;
  }
  if (Object.hasOwn(body, 'note') || Object.hasOwn(body, 'issue_note')) {
    options.note = body.note ?? body.issue_note;
  }
  if (
    Object.hasOwn(body, 'reprintedAt') ||
    Object.hasOwn(body, 'reprinted_at')
  ) {
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      const body = await withClient(async (client) => {
        const rows = await listPrintOrdersReceived(
          client,
          parseListOptions(event),
        );
        return { ok: true, rows };
      });
    ...
    if (event.httpMethod === 'POST') {
      ...
      const body = await withClient(async (client) => {
        const result = await syncPostPurchaseOrders(
          client,
          parseSyncOptions(event),
        );
    ...
    if (event.httpMethod === 'PUT') {
      ...
      const body = await withClient(async (client) => {
        const row = await updatePrintOrderLifecycleStatus(
          client,
          parseUpdateOptions(event),
        );
        return { ok: true, row };
      });
      return json(200, body);
    ...
    return json(500, {
      ok: false,
      error:
        error && error.message ? error.message : 'postpurchase-orders failed',
    });
  }
};
