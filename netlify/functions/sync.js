...
function getSyncContext(event, payload = {}) {
  return {
    clientId: String(
      payload.clientId || getHeader(event, 'x-printguard-client-id') || '',
    ),
    operator: String(
      payload.operator || getHeader(event, 'x-printguard-operator') || '',
    ),
    requestId: crypto.randomUUID(),
    source: String(
      payload.source ||
        getHeader(event, 'x-printguard-sync-source') ||
        'sync_api',
    ),
  };
}
...
function mapStockData(row) {
  ...
  const updatedAt = toIsoOrNull(
    data.updatedAt || data.updated_at || row?.updated_at,
  );
...
async function applyMovementUpsert(client, action, context) {
  ...
  if (!articleNumber) {
    const result = {
      accepted: false,
      reason: 'missing_movement_article_number',
    };
    await auditStockWrite(client, {
      entity: 'movement',
      key: action.key,
  ...
  const parentTombstone = await getStockTombstone(
    client,
    'item',
    articleNumber,
  );
  if (!parent || parent.deleted_at || parentTombstone?.deleted_at) {
    const result = {
      accepted: false,
      reason: 'missing_or_deleted_parent_item',
    };
    await auditStockWrite(client, {
      entity: 'movement',
      key: action.key,
...
async function applyStockDelete(client, action, context) {
  ...
  await writeStockTombstone(
    client,
    action,
    existing?.data || tombstone?.payload || null,
  );
...
async function applyStockAction(client, action, context) {
  if (action.action === 'delete')
    return applyStockDelete(client, action, context);
...
async function batchUpsertColoradoRollEvents(client, rollEvents) {
  const valid = rollEvents.filter(
    (event) => event?.id && event?.machineId && event?.type,
  );
  for (const chunk of chunkArray(valid, 300)) {
    const params = [];
    const valuesSql = chunk
...
exports.handler = async function handler(event) {
  ...
  if (!conn)
    return resp(500, { ok: false, error: 'Missing NEON_DATABASE_URL' });
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      const coRecords = (
        await client.query(
          'select data, updated_at from public.pg_co_records order by timestamp asc',
        )
      ).rows.map((r) => ({
        ...(r.data || {}),
        updatedAt: r.data?.updatedAt || r.updated_at || null,
      }));
      const coloradoRollStates = (
        await client.query(
          'select data, updated_at from public.pg_colorado_roll_states order by machine_id asc',
        )
      ).rows.map((r) => ({
        ...(r.data || {}),
        updatedAt: r.data?.updatedAt || r.updated_at || null,
      }));
      const coloradoRollEvents = (
        await client.query(
          'select data, updated_at from public.pg_colorado_roll_events order by timestamp asc, id asc',
        )
      ).rows.map((r) => ({
        ...(r.data || {}),
        updatedAt: r.data?.updatedAt || r.updated_at || null,
    ...
    if (event.httpMethod === 'POST') {
      ...
      const movements = Array.isArray(payload.movements)
        ? payload.movements
        : [];
      const stockActions = Array.isArray(payload.stockActions)
        ? payload.stockActions
        : [];
      const coRecords = Array.isArray(payload.coRecords)
        ? payload.coRecords
        : [];
      const coloradoRollStates = Array.isArray(payload.coloradoRollStates)
        ? payload.coloradoRollStates
        : [];
      const coloradoRollEvents = Array.isArray(payload.coloradoRollEvents)
        ? payload.coloradoRollEvents
        : [];

      await client.query('begin');

      const rejectedLegacyItems = await rejectLegacyStockPayload(
        client,
        'item',
        items,
        context,
      );
      const rejectedLegacyMovements = await rejectLegacyStockPayload(
        client,
        'movement',
        movements,
        context,
      );
      const stockActionSummary = await applyStockActions(
        client,
        stockActions,
        context,
      );
      const acceptedItemActions = stockActionSummary.results.filter(
        (result) => result.accepted && result.entity === 'item',
      ).length;
      const acceptedMovementActions = stockActionSummary.results.filter(
        (result) => result.accepted && result.entity === 'movement',
      ).length;
      const upsertedCoRecords = await batchUpsertCoRecords(client, coRecords);
      const upsertedColoradoRollStates = await batchUpsertColoradoRollStates(
        client,
        coloradoRollStates,
      );
      const upsertedColoradoRollEvents = await batchUpsertColoradoRollEvents(
        client,
        coloradoRollEvents,
      );
    ...
    if (event.httpMethod === 'DELETE') {
      ...
      if (kind === 'movement') {
        ...
        const deleteResult = await applyStockAction(
          client,
          {
            action: 'delete',
        ...
        const deleteResult = await applyStockAction(
          client,
          {
            ...
            operator: context.operator,
            payload: {
              articleNumber: normalizeArticleNumber(key),
              deletedAt: updatedAt,
              updatedAt,
            },
