...
export function normalizeStockAction(rawAction, context = {}) {
  ...
  const key = entity === "item"
    ? normalizeArticleNumber(input.key || input.articleNumber || payload.articleNumber)
  ...
  const updatedAt = toIsoOrNull(input.updatedAt || payload.updatedAt || payload.updated_at || payload.timestamp);
  if (!updatedAt) {
  ...
  return {
    ...
    action: {
      action,
      actionId: String(input.actionId || input.idempotencyKey || `${entity}:${key}:${updatedAt}:${action}`),
