  ...
  return `
    ...
    incoming_pipeline as (
      select
        ...
        p.source_xml_path,
        p.source_month,
        p.physically_printed_at,
        p.physically_printed_by,
        p.physically_printed_station,
        p.physically_printed_scan_id,
        p.physically_printed_batch_id,
    ...
    pipeline_base as (
      select
        ...
        pr.source_xml_path,
        pr.source_month,
        pr.physically_printed_at,
        pr.physically_printed_by,
        pr.physically_printed_station,
        pr.physically_printed_scan_id,
        pr.physically_printed_batch_id,
...
function mapPipelineRow(row) {
  ...
  return {
    ...
    physicallyPrintedAt: toIso(row.physically_printed_at),
    physicallyPrintedBy: row.physically_printed_by || '',
...
function addStatusFilter(where, value) {
  ...
  if (status === 'physically_printed' || status === 'printed_out') {
    where.push(`physically_printed_at is not null`);
    return;
  }
  if (status === 'processed') {
