...
async function ensureSchema(client) {
  ...
  await client.query(`alter table print_orders_received add column if not exists submit_tool_processed_at timestamptz null`);
  await client.query(`create index if not exists print_orders_received_order_number_idx on print_orders_received (order_number)`);
  await client.query(`create index if not exists print_orders_received_customer_order_id_idx on print_orders_received (customer_order_id)`);
  await client.query(`create index if not exists print_orders_received_submit_tool_at_idx on print_orders_received (submit_tool_at)`);
...
async function listTxtFiles(folderPath) {
  ...
  try {
    ...
    if (error && (error.code === 'EACCES' || error.code === 'EPERM')) {
      throw new Error(`Cannot access Submit Tool log folder: ${folderPath}. Check NAS permissions and the Windows user running the task.`);
    }
    throw error;
  }
...
async function main() {
  ...
  const root = String(args.root || process.env.SUBMIT_TOOL_LOG_ROOT || '').trim().replace(/[\\/]+$/, '');
  if (!root) {
