(function attachPrintGuardStockDomain(root, factory) {
  ...
  function createStockDomainModule() {
    'use strict';
    ...
    function getMovementsForArticle(movements, articleNumber) {
      return (movements || []).filter(
        (move) => move.articleNumber === articleNumber,
      );
    }

    function computeStockSummary(item, movements, config, stockReports, now) {
      return stockReports.buildStockSummary(
        item,
        movements,
        { weeksN: config.weeksN },
        now || new Date(),
      );
    }
    ...
    function getMovementTypeClass(movType) {
      return (
        { receipt: 'receipt-c', issue: 'issue-c', stocktake: 'stocktake-c' }[
          movType
        ] || ''
      );
    }
