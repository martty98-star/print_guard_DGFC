(function attachChecklistApi(root, factory) {
  ...
  function createChecklistApi() {
    ...
    async function request(method, url, body, options) {
      ...
      if (!response.ok || payload.ok === false) {
        const error = new Error(
          payload.error || 'Checklist request failed: ' + response.status,
        );
    ...
    function listChecklistItems(options) {
      return request(
        'GET',
        '/.netlify/functions/checklist-items',
        null,
        options,
      );
    }
    ...
    function completeChecklistOccurrence(payload, options) {
      return request(
        'POST',
        '/.netlify/functions/checklist-completions',
        payload,
        options,
      );
    }
    ...
    function listChecklistCompletions(options) {
      ...
      return request(
        'GET',
        '/.netlify/functions/checklist-completions?limit=' +
    ...
    function evaluateChecklistReminders(payload, options) {
      return request(
        'POST',
        '/.netlify/functions/checklist-evaluate',
        payload || {},
        options,
      );
    }
