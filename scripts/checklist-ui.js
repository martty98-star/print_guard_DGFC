(function attachChecklistUi(root, factory) {
  ...
  function createChecklistUi() {
    ...
    const ChecklistApi =
      (typeof window !== 'undefined' && window.PrintGuardChecklistApi) || null;
    const ChecklistDomain =
      (typeof window !== 'undefined' && window.checklistDomain) || null;
    const ChecklistState =
      (typeof window !== 'undefined' && window.PrintGuardChecklistState) ||
      null;
    const ChecklistRender =
      (typeof window !== 'undefined' && window.PrintGuardChecklistRender) ||
      null;
    const ChecklistEvents =
      (typeof window !== 'undefined' && window.PrintGuardChecklistEvents) ||
      null;
    ...
    async function refreshChecklist(force) {
      ...
      try {
        ...
        state.completions = Array.isArray(logResponse.completions)
          ? logResponse.completions
          : [];
        state.loaded = true;
        ...
        state.showToast &&
          state.showToast(
            error && error.message
              ? error.message
              : ChecklistState.t(state, 'checklist.error.load'),
            'error',
          );
      } finally {
        state.loading = false;
    ...
    async function saveChecklist() {
      try {
        const authHeaders =
          typeof state.adminHeaders === 'function' ? state.adminHeaders() : {};
        ...
        if (payload.id) {
          await ChecklistApi.updateChecklistItem(requestBody, {
            fetchImpl: state.fetchImpl,
            headers: authHeaders,
          });
        } else {
          await ChecklistApi.createChecklistItem(requestBody, {
            fetchImpl: state.fetchImpl,
            headers: authHeaders,
          });
        }
        ...
        state.showToast &&
          state.showToast(
            ChecklistState.t(state, 'checklist.toast.saved'),
            'success',
          );
        await refreshChecklist(true);
      } catch (error) {
        const message =
          typeof state.adminErrorMessage === 'function'
            ? state.adminErrorMessage(error)
            : error && error.message
              ? error.message
              : ChecklistState.t(state, 'checklist.error.save');
    ...
    async function toggleChecklist(id) {
      ...
      try {
        ...
        state.showToast &&
          state.showToast(
            ChecklistState.t(state, 'checklist.toast.updated'),
            'success',
          );
        await refreshChecklist(true);
      } catch (error) {
        const message =
          typeof state.adminErrorMessage === 'function'
            ...
            : error && error.message
              ? error.message
              : ChecklistState.t(state, 'checklist.error.update');
        state.showToast && state.showToast(message, 'error');
      }
    }
    ...
    async function completeChecklist(item) {
      ...
      if (
        ChecklistState.getCompletedOccurrenceKeys(state).has(
        ...
        const result = await ChecklistApi.completeChecklistOccurrence(
          {
            ...
            actor: ChecklistState.getActor(state),
            device_id:
              state.cfg && state.cfg.deviceId ? state.cfg.deviceId : 'device',
          },
          { fetchImpl: state.fetchImpl },
        );
        state.showToast &&
          state.showToast(
            result && result.alreadyCompleted
              ? 'Checklist occurrence already completed'
              : 'Checklist occurrence completed',
            'success',
          );
        await refreshChecklist(true);
      } catch (error) {
    ...
    async function deleteChecklist(id) {
      const runDelete = async function runDelete() {
        try {
          const authHeaders =
            typeof state.adminHeaders === 'function'
              ? state.adminHeaders()
              : {};
          ...
          state.showToast &&
            state.showToast(
              ChecklistState.t(state, 'checklist.toast.deleted'),
              'success',
            );
          await refreshChecklist(true);
        } catch (error) {
          const message =
            typeof state.adminErrorMessage === 'function'
              ...
              : error && error.message
                ? error.message
                : ChecklistState.t(state, 'checklist.error.delete');
          state.showToast && state.showToast(message, 'error');
        }
      };
      ...
      if (typeof state.showConfirm === 'function') {
        state.showConfirm(
          ChecklistState.t(state, 'checklist.confirm.delete'),
          runDelete,
        );
        return;
      }
    ...
    async function runManualEvaluation() {
      try {
        ...
        state.showToast &&
          state.showToast(
            ChecklistState.t(state, 'checklist.toast.evaluate') +
              ': ' +
              summary,
            'success',
          );
        ...
        const message =
          typeof state.adminErrorMessage === 'function'
            ...
            : error && error.message
              ? error.message
              : ChecklistState.t(state, 'checklist.error.evaluate');
        state.showToast && state.showToast(message, 'error');
      }
    }
