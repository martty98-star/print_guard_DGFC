...
(() => {
  ...
  function renderActionNeeded(row) {
    if (row.reprintPending) return '<span class="pp-pipeline-badge reprint">Reprint pending</span><span>Mark the reprint as done after it is printed.</span>';
    if (isMissingProcessedXml(row)) return '<span class="pp-pipeline-badge missing">Action needed</span><span>Processed XML is missing after 30 minutes.</span>';
    return '<span class="pp-pipeline-badge done">No action needed</span><span>Order is tracked. Request a reprint only if needed.</span>';
  }
    ...
    return `<div class="pp-reprint-history">
      ${entries.map((entry) => `<div class="pp-reprint-history-entry">
        ...
        <div class="pp-reprint-history-meta">
          <span>By: ${esc(entry.requestedBy || '-')}</span>
        ...
        <div class="pp-file-actions">
          ${entry.status === 'pending' ? `<button class="btn-sm" type="button" data-resolve-reprint-order-id="${esc(entry.orderId || '')}" data-resolve-print-file-path="${esc(entry.printFilePath || '')}">Mark reprinted</button><button class="btn-sm" type="button" data-delete-reprint-request-id="${esc(entry.id || '')}" data-delete-reprint-admin="false">Cancel request</button>` : ''}
          ${options.isAdmin ? `<button class="btn-sm admin-only" type="button" data-delete-reprint-request-id="${esc(entry.id || '')}" data-delete-reprint-admin="true">Delete</button>` : ''}
        </div>
      </div>`).join('')}
    </div>`;
  }
    ...
    return files.map((file, index) => {
      ...
      return `<div class="pp-file-block">
        <div class="pp-file-row">
          ...
          ${pending ? '<span class="pp-pipeline-badge reprint">Reprint pending</span>' : ''}
        </div>
        <div class="pp-file-actions">
          ...
          ${pending ? '' : `<button class="btn-sm" type="button" data-reprint-order-id="${esc(orderId || '')}" data-reprint-order-name="${esc(row.orderName || '')}" data-print-file-path="${esc(path)}" data-print-file-label="${esc(label)}" ${reprintDisabled ? 'disabled' : ''}>Request reprint</button>`}
        </div>
  ...
  function renderFullReprintAction(row, options) {
    ...
    if (pending) {
      return `<button class="btn-sm" type="button" data-resolve-reprint-order-id="${esc(orderId || '')}" data-resolve-print-file-path="">Mark full reprinted</button>`;
    }
    return `<button class="btn-sm" type="button" data-reprint-order-id="${esc(orderId || '')}" data-reprint-order-name="${esc(row.orderName || '')}" data-print-file-path="" data-print-file-label="Full order" ${orderId ? '' : 'disabled'}>Full reprint</button>`;
  }
  ...
  function renderProcessedReprintRecords(row, esc) {
    ...
    return `<div class="pp-reprint-records">
      <div class="pp-section-label">Processed reprint XML: ${esc(row.reprintRecordCount || records.length)}</div>
      ${records.map((record) => {
    ...
    return `<div class="pp-processed-list">${rows.map((row) => `
      <article class="pp-processed-card">
        ...
        <section class="pp-card-section">
          ${renderProcessedReprintRecords(row, esc) || '<div class="pp-section-label">Reprint requests / history</div><div class="pp-order-sub">No processed reprint XML linked.</div>'}
        </section>
        <details class="pp-card-section pp-tech-details">
          <summary>Technical details</summary>
          <div class="pp-xml-source">
            <span>${esc(row.xmlFileName || '-')}</span>
            ${row.sourceXmlPath ? `<button class="btn-sm" type="button" data-copy-path="${esc(row.sourceXmlPath || '')}">Copy XML path</button>` : ''}
          </div>
