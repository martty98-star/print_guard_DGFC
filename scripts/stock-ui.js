(function attachPrintGuardStockUI(global) {
  ...
  function renderStockOverview(deps) {
    ...
    const statusCounts = StockDomain.getStockStatusCounts(S.items, computeStock);
    ...
    list.querySelectorAll('.item-card').forEach(c => {
      ...
      c.addEventListener('keydown', e => { if (e.key === 'Enter') onOpenStockDetail(c.dataset.article); });
    });
  }
  ...
  function buildMovementRows(deps) {
    ...
    return StockDomain.buildMovementReplayRows(moves).reverse().slice(0, 50).map(({ move: m, after }) => {
      const typeLabel = ({
  ...
  function buildStockHistoryTable(deps) {
    ...
    const html = StockDomain.buildStockHistoryRows(moves).reverse().slice(0, 100).map(({ move: m, after, delta }) => {
