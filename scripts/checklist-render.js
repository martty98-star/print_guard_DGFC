(function attachChecklistRender(root, factory) {
  ...
  function createChecklistRenderModule() {
    'use strict';
    ...
    function getVisibleOccurrence(item) {
      const getOccurrence =
        ChecklistDomain.getVisibleChecklistOccurrence ||
        ChecklistDomain.getNextChecklistOccurrence;
    ...
    function getCurrentLocalDateKey(state) {
      const timeZone =
        (state.items && state.items[0] && state.items[0].timeZone) ||
        'Europe/Prague';
    ...
    function isDateInRange(dateKey, range) {
      ...
      return (
        String(dateKey || '') >= range.start &&
        String(dateKey || '') <= range.end
      );
    }
    ...
    function buildLogEntries(state, range, onlyMissing) {
      ...
      if (!onlyMissing) {
        (state.completions || []).forEach((row) => {
          ...
          if (!parsed.localDate || !isDateInRange(parsed.localDate, range))
            return;
          const entry = {
            type: 'completed',
          ...
          if (!summaryByChecklist.has(summaryKey)) {
            summaryByChecklist.set(summaryKey, {
              title: entry.title,
              completed: 0,
              expected: 0,
            });
      ...
      (state.items || []).forEach((item) => {
        ...
        if (onlyMissing === false && !isDateInRange(visible.localDate, range))
          return;
      ...
      const groupedDays = Array.from(byDay.entries())
        ...
        .map(([dateKey, entries]) => ({
          ...
          entries: entries
            ...
            .sort(
              (a, b) =>
                String(a.localTime || '').localeCompare(
                  String(b.localTime || ''),
      ...
      const summaries = Array.from(summaryByChecklist.values()).sort((a, b) =>
        a.title.localeCompare(b.title),
      );
    ...
    function renderChecklistList(state) {
      ...
      host.innerHTML = state.items
        .map((item) => {
          ...
          const enabledLabel = item.enabled
            ? ChecklistState.t(state, 'checklist.status.enabled')
            : ChecklistState.t(state, 'checklist.status.disabled');
    ...
    function renderChecklistLog(state) {
      ...
      <div class="checklist-log-summary">
        ...
        <div class="checklist-log-summary-list">
          ...
          `,
                  ...
                  .join('')
              : '<div class="checklist-log-summary-empty">No summary data</div>'
          }
        </div>
      </div>
      ...
      ${
        ...
        `,
          ...
          : `<div class="empty-state"><h3>No checklist completions yet</h3><p>Completed occurrences will appear here.</p></div>`
      }
    `;
