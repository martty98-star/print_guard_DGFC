...
function getConfigFromPayload(payload, cliArgs) {
  ...
  return {
    inkCost:
      cliArgs['ink-cost'] != null
        ? Number(cliArgs['ink-cost'])
        : Number(settings.inkCost || 0),
    mediaCost:
      cliArgs['media-cost'] != null
        ? Number(cliArgs['media-cost'])
        : Number(settings.mediaCost || 0),
    costCurrency: String(
      cliArgs['cost-currency'] || settings.costCurrency || 'CZK',
    ).toUpperCase(),
  };
}
...
function buildCsv(rows, hasCosts) {
  return rowsToCsv(rows, [
    ...
    },
    {
      ...
      header: 'days_elapsed',
      value: (row) =>
        row.daysElapsed == null ? '' : fmtNumber(row.daysElapsed, 2),
    },
    {
      key: 'ink_total_l_to',
      header: 'ink_total_l_to',
      value: (row) =>
        row.inkTotalLTo == null ? '' : fmtNumber(row.inkTotalLTo, 3),
    },
    {
      key: 'media_total_m2_to',
      header: 'media_total_m2_to',
      value: (row) =>
        row.mediaTotalM2To == null ? '' : fmtNumber(row.mediaTotalM2To, 1),
    },
    {
      key: 'ink_used_l',
      header: 'ink_used_l',
      value: (row) => (row.inkUsedL == null ? '' : fmtNumber(row.inkUsedL, 3)),
    },
    {
      key: 'media_used_m2',
      header: 'media_used_m2',
      value: (row) =>
        row.mediaUsedM2 == null ? '' : fmtNumber(row.mediaUsedM2, 1),
    },
    {
      key: 'ink_per_m2',
      header: 'ink_per_m2',
      value: (row) => (row.inkPerM2 == null ? '' : fmtNumber(row.inkPerM2, 6)),
    },
    {
      key: 'ink_cost',
      header: 'ink_cost',
      value: (row) =>
        hasCosts && row.inkCost != null ? fmtNumber(row.inkCost, 2) : '',
    },
    {
      key: 'media_cost',
      header: 'media_cost',
      value: (row) =>
        hasCosts && row.mediaCost != null ? fmtNumber(row.mediaCost, 2) : '',
    },
    {
      key: 'total_cost',
      header: 'total_cost',
      value: (row) =>
        hasCosts && row.totalCost != null ? fmtNumber(row.totalCost, 2) : '',
    },
    {
      key: 'cost_per_m2',
      header: 'cost_per_m2',
      value: (row) =>
        row.costPerM2 == null ? '' : fmtNumber(row.costPerM2, 4),
    },
  ]);
}
...
function main() {
  ...
  if (!args.input) {
    fail(
      'Usage: node scripts/export-colorado-monthly.js --input <backup.json> [--output <file.csv>] [--month YYYY-MM] [--ink-cost 0] [--media-cost 0] [--cost-currency SEK]',
    );
  }

  const payload = loadInputJson(args.input);
  const coRecords = Array.isArray(payload.coRecords)
    ? payload.coRecords.filter((record) => record?.id && record?.machineId)
    : [];
  if (!coRecords.length) {
    fail('No Colorado records found in input JSON.');
  }

  const range = args.month
    ? getRangeForMonth(args.month)
    : getCurrentMonthRange();
  const config = getConfigFromPayload(payload, args);
  const rows = buildColoradoMonthlySummary(
    coRecords,
    config,
    range,
    DEFAULT_MACHINES,
