...
function getVariable(block, name) {
  for (const variable of getBlocks(block, 'Variable')) {
    if (
      getTag(variable, 'Name').toLowerCase() ===
...
function parseProcessedPrintOrderXml(xml, sourceXmlPath, sourceMonth) {
  ...
  const xmlFileName =
    getTag(xml, 'XmlFileName') || path.basename(sourceXmlPath);
...
async function listMonthFolders(root, options) {
  ...
  if (options.full) {
    ...
    const folders = entries
      ...
      .sort();
    console.log(
      `[processed-orders] selected folders: ${folders.join(',') || '(none)'}`,
    );
    return folders;
  }
...
async function listXmlFiles(folderPath, options) {
  ...
  try {
    ...
    if (error && error.code === 'ENOENT') {
      console.log(
        `[processed-orders] XML files found: 0 (${folderPath} missing)`,
      );
      return [];
    }
    if (error && (error.code === 'EACCES' || error.code === 'EPERM')) {
      throw new Error(
        `Cannot access processed XML folder: ${folderPath}. Check NAS permissions and the scheduled-task Windows user.`,
      );
    }
    throw error;
  }
  ...
  console.log(
    `[processed-orders] filtering XML files by modified time: last ${options.days} days`,
  );
  const recentFiles = [];
  ...
  console.log(
    `[processed-orders] XML files selected after days filter: ${recentFiles.length}`,
  );
  return recentFiles;
}
...
async function listKnownHashesForFiles(client, files, options) {
  ...
  console.log(
    `[processed-orders] loading known XML hashes from DB: ${files.length} paths`,
  );
...
async function syncProcessedPrintOrders(client, options) {
  ...
  if (options.file) {
    ...
    try {
      ...
      const knownHashes = await listKnownHashesForFiles(
        client,
        [options.file],
        options,
      );
      ...
      console.log(
        `[processed-orders] DB upsert start: ${order.orderName} (${order.xmlFileName || path.basename(options.file)})`,
      );
  ...
  for (const month of months) {
    ...
    for (let index = 0; index < files.length; index += 1) {
      ...
      try {
        console.log(
          `[processed-orders] parsing XML ${index + 1}/${files.length}: ${filePath}`,
        );
        const xml = await withTimeout(
        ...
        if (knownHashes.get(filePath) === sourceXmlHash) {
          console.log(
            `[processed-orders] unchanged XML skip ${index + 1}/${files.length}: ${filePath}`,
          );
          stats.skippedUnchanged += 1;
          continue;
        }
        ...
        if (!order.orderName) throw new Error('Missing <Name>');
        console.log(
          `[processed-orders] DB upsert start: ${order.orderName} (${order.xmlFileName || path.basename(filePath)})`,
        );
        const result = await withTimeout(
          upsertProcessedPrintOrder(client, order),
          `DB upsert ${filePath}`,
...
async function main() {
  ...
  const root = cleanString(
    args.root ||
      process.env.PROCESSED_PRINT_ORDERS_ROOT ||
      process.env.ONYX_PROCESSED_XML_ROOT,
  );
  ...
  const options = {
    ...
    file,
    days: toPositiveInteger(
      args.days || process.env.PROCESSED_PRINT_ORDERS_DAYS,
      2,
    ),
    full: Boolean(args.full),
    timeoutMs: toPositiveInteger(
      args.timeoutMs || process.env.PROCESSED_PRINT_ORDERS_TIMEOUT_MS,
      DEFAULT_OPERATION_TIMEOUT_MS,
    ),
  };
  ...
  console.log(
    `[processed-orders] sync start root=${options.root} scope=${options.file || (options.full ? 'full' : options.month || `${options.days}d`)}`,
  );
  ...
  console.log(
    `[processed-orders] months=${result.monthsScanned} scannedXmlFiles=${result.scannedXmlFiles} insertedOrders=${result.insertedOrders} updatedOrders=${result.updatedOrders} skippedUnchanged=${result.skippedUnchanged} errors=${result.errors}`,
  );
}
