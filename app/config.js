...
(function attachPrintGuardAppConfig(global) {
  const APP_VERSION = 'printguard-8.1.7';
  ...
  const cfg = {
    get theme() {
      return normalizeTheme(ls('pg_theme') || 'light');
    },
    set theme(v) {
      ls('pg_theme', applyTheme(v));
    },
    get weeksN() {
      return parseInt(ls('pg_weeks') || '8', 10);
    },
    set weeksN(v) {
      ls('pg_weeks', v);
    },
    get rollingN() {
      return parseInt(ls('pg_rolling') || '8', 10);
    },
    set rollingN(v) {
      ls('pg_rolling', v);
    },
    get inkCost() {
      return parseFloat(ls('pg_ink_cost') || '0');
    },
    set inkCost(v) {
      ls('pg_ink_cost', v);
    },
    get mediaCost() {
      return parseFloat(ls('pg_media_cost') || '0');
    },
    set mediaCost(v) {
      ls('pg_media_cost', v);
    },
    ...
    get userName() {
      return ls('pg_user_name') || '';
    },
    set userName(v) {
      ls('pg_user_name', v);
    },
    get role() {
      return ls('pg_role') || 'operator';
    },
    set role(v) {
      ls('pg_role', v);
    },
    get adminPin() {
      return sessionStorage.getItem('pg_admin_pin') || '';
    },
    set adminPin(v) {
    ...
    get postPurchasePin() {
      return sessionStorage.getItem('pg_postpurchase_pin') || '';
    },
