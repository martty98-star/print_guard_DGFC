...
(function attachPrintGuardReporting(global) {
  function createReporting(deps) {
    const {
      APP_VERSION, MACHINES, Reports, S, ST_CORECS, ST_ITEMS, ST_MOVES, ST_SETTINGS,
    ...
    function exportCSVRawCo() {
      const rows = [csvRow(['id','machine','timestamp','ink_total_l','media_total_m2','note','created_at'])];
      [...S.coRecords].sort((a,b) => new Date(a.timestamp)-new Date(b.timestamp)).forEach(r => {
        rows.push(csvRow([r.id, r.machineId, fmtExportDateTime(r.timestamp), r.inkTotalLiters, r.mediaTotalM2, r.note||'', fmtExportDateTime(r.createdAt)]));
      });
    ...
    function exportCSVStockLevels() {
      ...
      const rows = Reports.stock.buildStockLevels(S.items, S.movements, { weeksN: cfg.weeksN }, exportedAt);
      const csv = Reports.csv.rowsToCsv(rows, [
    ...
    async function exportCSVPrintLog() {
      try {
        const rows = [csvRow(['ready_at','printer','job_name','result','media_type','printed_area_m2','ink_total_l','ink_source','ink_cyan_l','ink_magenta_l','ink_yellow_l','ink_black_l','ink_white_l','derived_ink_l_per_m2','estimate_interval_from','estimate_interval_to','duration_sec'])];
        const allRows = [];
        let offset = 0;
        while (true) {
        ...
        allRows.forEach(row => {
          ...
          rows.push(csvRow([
            fmtExportDateTime(row.readyAt), mapPrinterName(row.printerName), row.jobName || '', printResultLabel(row.result), row.mediaType || '',
            ...
            direct.channels?.cyan == null ? '' : fmtN(direct.channels.cyan, 3),
            direct.channels?.magenta == null ? '' : fmtN(direct.channels.magenta, 3),
            direct.channels?.yellow == null ? '' : fmtN(direct.channels.yellow, 3),
            direct.channels?.black == null ? '' : fmtN(direct.channels.black, 3),
            direct.channels?.white == null ? '' : fmtN(direct.channels.white, 3),
            ink.source === 'estimated' && ink.estimatedInkPerM2 !== null ? fmtN(ink.estimatedInkPerM2, 6) : '',
            fmtExportDateTime(interval?.from), fmtExportDateTime(interval?.to),
    ...
    async function exportJSON() {
      const data = {
        ...
        settings: [{ key: 'config', weeksN: cfg.weeksN, rollingN: cfg.rollingN, inkCost: cfg.inkCost, mediaCost: cfg.mediaCost, costCurrency: cfg.costCurrency, savedAt: new Date().toISOString() }],
      };
    ...
    return {
      exportCSVCombinedLifetimeCo, exportCSVCurrentMonthCo, exportCSVIntervals,
      exportCSVPrintLog, exportCSVRawCo, exportCSVStock, exportCSVStockLevels,
      exportJSON, handleImportJSON,
    };
  }
