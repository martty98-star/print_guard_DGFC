...
(function attachPrintGuardPush(global) {
  function createPush(deps) {
    const {
      Reports, buildPushSubscriptionPayload, cfg, getPushDeviceName, getPushEndpointSuffix,
      persistPushSubscription, showToast, urlBase64ToUint8Array,
    } = deps;
    ...
    async function enablePushNotifications() {
      try {
        ...
        const vapidPublicKey = typeof global.VAPID_PUBLIC_KEY === 'string' ? global.VAPID_PUBLIC_KEY.trim() : '';
        if (!vapidPublicKey) {
          showToast('Chybí VAPID public key.', 'error');
          return;
        }
        console.log('[Push] VAPID_PUBLIC_KEY presence', { exists: Boolean(global.VAPID_PUBLIC_KEY), length: vapidPublicKey.length });
        ...
        console.log('[Push] subscribe start', {
          ...
          endpointSuffix: getPushEndpointSuffix(subscription && typeof subscription.endpoint === 'string' ? subscription.endpoint : ''),
        });
        if (subscription) {
          const existingPayload = buildPushSubscriptionPayload(subscription);
          if (existingPayload) {
            console.log('[Push] reusing existing subscription', { deviceName, endpointSuffix: getPushEndpointSuffix(existingPayload.endpoint) });
            await persistPushSubscription(existingPayload);
            showToast('Push notifikace byly povoleny.', 'success');
            return;
          }
          try {
            console.warn('[Push] unsubscribing stale subscription', { deviceName, endpointSuffix: getPushEndpointSuffix(subscription.endpoint) });
        ...
        if (!subscription) {
          ...
          if (applicationServerKey.length !== 65) throw new Error('VAPID public key has invalid length.');
          subscription = await registration.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey });
          console.log('[Push] created new subscription', { deviceName, endpointSuffix: getPushEndpointSuffix(subscription.endpoint) });
        }
    ...
    async function sendStockNotifications(options = {}) {
      ...
      try {
        const result = await Reports.notificationDispatch.evaluateStockAlerts({ weeksN: cfg.weeksN, trigger });
        if (!silent) {
          showToast(`Stock notifikace: nové ${result.sentAlerts || 0}, beze změny ${result.skippedAlerts || 0}.`, 'success');
        }
        return result;
      } catch (error) {
        console.error('[Push] stock notifications failed', error);
        if (!silent) showToast(error?.message || 'Odeslání stock notifikací selhalo.', 'error');
        return null;
      }
    }
